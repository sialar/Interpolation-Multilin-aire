...
*/
    ...
    interp->computeLiMinus1Fi(sizeX,sizeY);
    ...
    for (int i=0; i<int(testPointsX.size()); i++)
    {
        for (int j=0; j<int(testPointsY.size()); j++)
        {
            val = interp->lagrangeInterpolation_2D_iterative(testPointsX[i],testPointsY[j],sizeX,sizeY);
            estimate.push_back(val);
            cout << val << " ";
        }
        cout << endl;
    ...
    val = interp->lagrangeInterpolation_2D_iterative(Utils::randomValue(-1,1),Utils::randomValue(-1,1),sizeX,sizeY);
