...
###========================================= 
    ...
    """
    Récupérer les points de test, on peut faire dans le terminal, ex, on tape :
    
    fichier="RelativeError_AP2_Cocagne_Tucker_GADO_withGreedy_eps1E_9_13300p_macro_fission0"
    
    Puis:
    awk '{print "("$2 ", "  $3 ", " $4 ", " $5 ", " $6 ")"}' $fichier > reference_points.dat
    """
    listOfReferencePoints = []
    NameFile = "reference_points.dat"
    with open(NameFile) as f:
        for line in f:   
            point = ast.literal_eval(line)
            listOfReferencePoints.append(point)

 ###=================================================== 
 
    ##point = (150.0, 280.0, 0.6,  8.74176476e-06, 1.0)
    ##point = (24000.0,	20.0,	0.40000000596,	1.74835295184e-05,	0.75)
    #point = (24500.0, 120.0, 0.899999976158, 1.18510197353e-05, 	9.99999997475e-07)
    #value_eva = evaluate(listOfBasicFctsUsingLagrangeInterpolation[csName], listOfFinalCoefIndexes_arr[csName], FinalTuckerCoeffs[csName], dimension, point) 
    #print value_eva
    ##0.00115876609925	0.00112518646401	0.00116008161272
...
###===================================================
    """
    Si on veut tester avec une seule section, sinon, mettre en commentaire 3 lignes :
    iSection = ...
 ...
 ###===================================================  
    ...
    listOfAP2= []
    NameFile = "AP2_macro_totale0.dat"
    with open(NameFile) as f:
        for line in f: 
            listOfAP2.append(float(line))
    ...
    for csName in listOfCrossSectionNamesWithArgs:        
        ...
        for i in range(len(listOfReferencePoints)):
            ...
            value_T = evaluate(listOfBasicFctsUsingLagrangeInterpolation[csName], listOfFinalCoefIndexes_arr[csName], FinalTuckerCoeffs[csName], dimension, point) #listOfTuckerDecomp[csName].evaluate(point)
            value_A = listOfAP2[i]
            ...
            print "count =", count
            print "point = ", point
            print "Tucker = %s, AP2 = %s" %(values_Tucker[count-1].real,values_AP2[count-1])
